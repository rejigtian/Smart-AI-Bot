...
| 维度 | Smart-AI-Bot | DroidRun | Midscene.js | AutoGLM |
|------|:----------------:|:--------:|:-----------:|:-------:|
...
Smart-AI-Bot: 截图 × 0.5 缩放 + 网格标注 + 品红十字 SoM → a11y 树文本 + Activity → LLM 决策
...
DroidRun 和 Midscene 都依赖 ADB，要求设备与 PC 在同一网络（或 USB 直连）。Smart-AI-Bot 的 Portal App 主动建立 WebSocket 连接，设备可以在任意位置（4G/5G/公司 WiFi），服务器部署在云端即可管理全球设备 — 这在多端 QA 场景（如设备农场）中是显著优势。
...
Smart-AI-Bot 在这一层有完整实现：套件创建 / 用例增删改 / 历史 Run 列表 / 单 Run 详情 / 步骤回放 / 星标参考案例 / HTML 报告导出。这是当前最直接的差异化。
