...
| 恢复级别 | `recovery_level` — 纯信号：仅当「动作签名 + 画面指纹」双双冻结时才累加（渐衰而非硬重置）。不再驱动 auto back/restart；唯一代码层动作是 L4 force-fail 兜底，`loop_task` 用例跳过该兜底、仅靠 `max_steps` 收尾 | `agent/memory.py` |
...
| 决策层 | 状态感知 stuck 检测（动作+画面双冻结才算卡住）+ 信任 AI 自主脱困，L4 force-fail 兜底 | ✅ |
| 前端 | 运行对比页面 | ✅ |
...
早期 `is_stuck()` 只比对动作签名，导致答题这类「每步都点同一个选项索引但分数/题目在前进」的任务被误判为死循环，进而触发代码层强制 `back` 弹出页面。现在 `is_stuck()` 额外比对每步的画面指纹（`record_state(ui_text)` 写入 `_state_signatures`）：只有「动作签名 + 画面指纹」在窗口内**双双冻结**才算卡住。重复动作但画面在变 = 真实进度。强制 back/restart 已移除，改为软化的提示让 AI 自主决定；真·死循环由 L4 force-fail（普通用例）或 `max_steps`（`loop_task` 用例）兜底。
